...
# pylint: disable=C0301, C0415, W0611, W1405
...
from app.adk_common.utils.evaluation_prompts import (
...
# @log_function_call
def _get_internal_prompt(mime_type: str, evaluation_criteria: str, reference_image_descriptions: Optional[list[str]], allow_collage: bool = False) -> str:
...
async def evaluate_media(
    media_bytes: bytes, mime_type: str, evaluation_criteria: str, reference_images: Optional[List[Tuple[genai.types.Part, str]]] = None, allow_collage: bool = False
) -> EvalResult:
    ...
    try:
        ...
        async with get_evaluation_semaphore():
            ...
            from app.adk_common.utils.gemini_utils import get_gemini_client
            ...
            if result.decision.lower() == "fail":
                log_message(f"Evaluation failed reason: {result.reason}", Severity.WARNING)
            else:
