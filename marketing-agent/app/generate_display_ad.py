...
# pylint: disable=C0301, W0102, W0104, W0718, W1309, W1405
...
from .adk_common.utils import ad_generation_constants, utils_agents
...
async def _create_display_ad_task(
    ...
    image_descriptions = ["ASSET SHEET: Follow the visual style and character look defined here."]
...
async def generate_display_ad(
    ...
    try:
        ...
        final_product_uri = (
            tool_context.state.get("PRODUCT_IMAGE_URI")
            or get_required_env_var("BACKUP_CATALOG_IMAGE_URL")
        )
        final_logo_uri = (
            tool_context.state.get("LOGO_IMAGE_URI")
        ...
        if result and result.get("status") == "success" and result.get("image_bytes"):
            ...
            # Log success
            log_message(f"[generate_display_ad] Success. New image URI: {generated_media.gcs_uri}", Severity.INFO)

            return {
                "status": "success",
                "detail": "Display Ad generated successfully.",
                "generated_image_uri": generated_media.gcs_uri,
                "evaluation": best_attempt_evaluation.model_dump() if best_attempt_evaluation else None
            }
